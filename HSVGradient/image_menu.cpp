...
void showMenu(std::ostream &os)
{
    ...
    os << "mandelbar) Choose to make a Mandelbar set.\n";
    ...
    os << "set-hsv-color-gradient) Smoothly set the RGB values for a range of slots in the color table.\n";
    ...
    while (choice != "quit")
    {
        ...
        if (!quiet)
        {
            ...
            if (choice == "quiet")
            {
                ...
                setMandelbarFractal(is, os, gptr);
            }
        }
        else
        {
...
void setMandelbarFractal( std::istream& is, std::ostream& os, NumberGrid*& grid ){
...
void setHSVColorGradient( std::istream& is, std::ostream& os, ColorTable& table ){
    ...
    int p1 = getInteger(is, os, "First position? ");
    int h1 = getInteger(is, os, "First hue? ");
    int s1 = getInteger(is, os, "First saturation? ");
    int v1 = getInteger(is, os, "First value? ");

    int p2 = getInteger(is, os, "Second position? ");
    int h2 = getInteger(is, os, "Second hue? ");
    int s2 = getInteger(is, os, "Second saturation? ");
    int v2 = getInteger(is, os, "Second value? ");

    table.insertHSVGradient(h1,s1,v1,h2,v2,s2,p1,p2);
}
