    ...
    for (int row = 0; row < h; row++)
    {
        for (int column = 0; column < w; column++)
        {
            for (int channel = 0; channel < 3; channel++)
            {
                ...
                int old_red = this->getChannel(row, column, 0);
                int old_green = this->getChannel(row, column, 1);
                int old_blue = this->getChannel(row, column, 2);

                double new_red = 0.393 * old_red + 0.769 * old_green + 0.189 * old_blue;
                double new_green = 0.349 * old_red + 0.686 * old_green + 0.168 * old_blue;
                double new_blue = 0.272 * old_red + 0.534 * old_green + 0.131 * old_blue;
